...
for n in range(N_ELBO_ITR):
    ELBO.append(model.estimate_log_model_evidence(N_ELBO).detach().numpy())
print("MF ELBO: {} +- {}".format(np.mean(ELBO), np.std(ELBO)/np.sqrt(float(N_ELBO_ITR))))
...
noise_input = NormalVariable(np.zeros((noise_inpt_size,)), 0.02*np.ones((noise_inpt_size,)), "noise_input", learnable=True)
...
for n in range(N_ELBO_ITR):
    ELBO.append(model.estimate_log_model_evidence(N_ELBO).detach().numpy())
print("Hierarchical ELBO: {} +- {}".format(np.mean(ELBO), np.std(ELBO)/np.sqrt(float(N_ELBO_ITR))))
...
for n in range(N_ELBO_ITR):
    ELBO.append(model.estimate_log_model_evidence(N_ELBO).detach().numpy())
print("PE ELBO: {} +- {}".format(np.mean(ELBO), np.std(ELBO)/np.sqrt(float(N_ELBO_ITR))))

#Plot results
plt.plot(loss_list1, label="MF")
plt.plot(loss_list2, label="Structured (PE)")
plt.plot(loss_list3, label="Structured (Hierarchical)")
plt.legend(loc="best")
plt.show()

sigmoid = lambda x: 1/(np.exp(-x) + 1)
image_grid = []
z_range = np.linspace(-4, 4, 10)
label_matrix = []
for z1a in z_range:
    image_row = []
    label_row = []
    ...
    label_matrix += [label_row]
print(np.transpose(np.array(label_matrix)))
