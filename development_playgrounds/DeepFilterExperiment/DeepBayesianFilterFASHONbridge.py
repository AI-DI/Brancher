...
for rep in range(N_rep):
    ...
    imagesGT.append([np.reshape(samples[imgt].detach().numpy(), (3, image_size, image_size))
                     for imgt in img])
    imagesNoise.append([np.reshape(samples[xt].detach().numpy(), (3, image_size, image_size))
                        for xt in x])
    ...
    for _ in range(5):
        ...
        im_list1.append([np.reshape(psamples[img[t]].detach().numpy(), (3, image_size, image_size))
                         for t in range(T)])
    ...
    for _ in range(5):
        psamples = model._get_posterior_sample(1)
        im_list2.append([np.reshape(psamples[img[t]].detach().numpy(), (3, image_size, image_size))
                         for t in range(T)])
...
import pickle
with open('F_bridge_results.pickle', 'wb') as f:
    pickle.dump(d, f)
