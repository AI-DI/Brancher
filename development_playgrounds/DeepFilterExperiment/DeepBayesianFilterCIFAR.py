...
for rep in range(N_rep):
    ...
    imagesGT.append([np.reshape(samples[img[t]].detach().numpy(), (3, 32, 32))
                   for t in range(T)])
    imagesNoise.append([np.reshape(samples[x[t]].detach().numpy(), (3, 32, 32))
                        for t in range(T)])
    ...
    images1.append([np.reshape(psamples[img[t]].detach().numpy(), (3, 32, 32))
                    for t in range(T)])
    # plt.show()
    ...
    images2.append([np.reshape(psamples[img[t]].detach().numpy(), (3, 32, 32))
                    for t in range(T)])
